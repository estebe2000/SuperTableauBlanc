import { appConfig, PROVIDER_PRESETS, DEFAULT_TOOL_CONFIGS, DEFAULT_SYSTEM_PROMPTS } from './config.js';
import { probeModelsEndpoint, checkActiveProviderStatus } from './api.js';

const TOOLS = [
  { id: 'vision', name: 'Vision & Décripteur (Images)', icon: '📸' },
  { id: 'voice', name: 'Voice AI (Transcription & CUA)', icon: '🎙️' },
  { id: 'formalizer', name: 'Reformulateur (Nuanceur de ton)', icon: '✍️' },
  { id: 'todo', name: 'Pas-à-Pas (Découpeur de tâches)', icon: '🧩' },
  { id: 'professor', name: 'Clarificateur de concepts (CUA)', icon: '💡' },
  { id: 'judge', name: 'Décrypteur d\'Intention', icon: '🧭' }
];
...
const DEFAULT_RESOLVED_DATA = {
  ...
  ilaas: {
    type: 'openai',
    modelsUrl: (typeof window !== 'undefined' ? window.location.origin : '') + '/proxy-ilaas/v1/models',
    chatUrl: (typeof window !== 'undefined' ? window.location.origin : '') + '/proxy-ilaas/v1/chat/completions'
  },
  albert: {
    type: 'openai',
...
export function initSettings() {
  ...
  function selectProvider(providerId) {
    ...
    if (config) {
      ...
      appConfig.model = config.defaultModel || appConfig.model;
    }

    const resolved = providerResolvedData[providerId];
    if (resolved) {
      appConfig.resolvedType = resolved.type;
      appConfig.resolvedModelsUrl = resolved.modelsUrl;
      appConfig.resolvedChatUrl = resolved.chatUrl;
    } else if (providerId === 'albert') {
      appConfig.resolvedType = 'openai';
      appConfig.resolvedModelsUrl = (typeof window !== 'undefined' ? window.location.origin : '') + '/proxy-albert/v1/models';
      appConfig.resolvedChatUrl = (typeof window !== 'undefined' ? window.location.origin : '') + '/proxy-albert/v1/chat/completions';
    } else if (providerId === 'ilaas') {
      appConfig.resolvedType = 'openai';
      appConfig.resolvedModelsUrl = (typeof window !== 'undefined' ? window.location.origin : '') + '/proxy-ilaas/v1/models';
      appConfig.resolvedChatUrl = (typeof window !== 'undefined' ? window.location.origin : '') + '/proxy-ilaas/v1/chat/completions';
    } else if (providerId === 'ollama') {
      appConfig.resolvedType = 'ollama';
      appConfig.resolvedModelsUrl = 'http://localhost:11434/api/tags';
      appConfig.resolvedChatUrl = 'http://localhost:11434/api/generate';
    }
  ...
  saveSettingsBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // Update all tools using this provider so they inherit the new URL and API key
      if (appConfig.tools) {
        Object.keys(appConfig.tools).forEach(toolKey => {
          if (appConfig.tools[toolKey].provider === providerId) {
            appConfig.tools[toolKey].url = url;
            appConfig.tools[toolKey].apiKey = apiKey;
            appConfig.tools[toolKey].resolvedType = result.type;
            appConfig.tools[toolKey].resolvedModelsUrl = result.url;
            appConfig.tools[toolKey].resolvedChatUrl = providerResolvedData[providerId].chatUrl;
          }
        });
      }
  ...
  // ─── 3. PROMPT EDITOR LOGIC ───
  const PROMPT_TOOLS = [
    { id: 'todo', name: 'Pas-à-Pas (Tâches)', icon: '🧩', variables: ['{task}', '{granularity}', '{granularity_desc}'] },
    { id: 'formalizer', name: 'Reformulateur', icon: '✍️', variables: ['{text}', '{tones}'] },
    { id: 'judge', name: 'Décrypteur d\'Intention', icon: '🧭', variables: ['{text}'] },
    { id: 'professor', name: 'Clarificateur (CUA)', icon: '💡', variables: ['{concept}', '{mode}', '{context}'] },
    { id: 'professorPlus', name: 'Studio Pédagogique CUA', icon: '🎓', variables: ['{theme}', '{cycle}', '{subject}'] },
    { id: 'vision', name: 'Vision & Décripteur', icon: '👁️', variables: ['{image_context}'] },
    { id: 'voice', name: 'Voice AI (Transcription)', icon: '🎙️', variables: ['{format}'] },
    { id: 'student', name: 'Tuteur Virtuel Élève', icon: '🧑‍🎓', variables: ['{text}', '{student_profile}'] }
  ];

  let activePromptTool = 'todo';
  ...
  const promptVariablesHints = document.getElementById('promptVariablesHints');
  ...
  const resetCurrentPromptBtn = document.getElementById('resetCurrentPromptBtn');
  const resetAllPromptsBtn = document.getElementById('resetAllPromptsBtn');

  function renderPromptTabs() {
    if (!promptTabsContainer) return;
    promptTabsContainer.innerHTML = '';
    PROMPT_TOOLS.forEach(tool => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `prompt-tab-btn ${tool.id === activePromptTool ? 'active' : ''}`;
      btn.innerHTML = `<span>${tool.icon}</span> <span>${tool.name}</span>`;
      btn.addEventListener('click', () => {
        if (promptEditorTextarea) {
          appConfig.prompts[activePromptTool] = promptEditorTextarea.value;
        }
        activePromptTool = tool.id;
        renderPromptTabs();
        loadActivePrompt();
      });
  ...
  function loadActivePrompt() {
    ...
    const toolInfo = PROMPT_TOOLS.find(t => t.id === activePromptTool) || PROMPT_TOOLS[0];
    if (activePromptLabel) {
      activePromptLabel.textContent = `Directive système pour : ${toolInfo.icon} ${toolInfo.name}`;
    }

    // Render variables hints
    if (promptVariablesHints) {
      promptVariablesHints.innerHTML = '<span style="font-weight:600;">Variables :</span>';
      toolInfo.variables.forEach(v => {
        const badge = document.createElement('span');
        badge.className = 'var-badge';
        badge.textContent = v;
        badge.title = `Cliquer pour insérer ${v} au curseur`;
        badge.addEventListener('click', () => {
          insertVariableAtCursor(v);
        });
        promptVariablesHints.appendChild(badge);
      });
    }

    const currentVal = appConfig.prompts[activePromptTool] || DEFAULT_SYSTEM_PROMPTS[activePromptTool] || '';
    promptEditorTextarea.value = currentVal;
    updateCharCount();
  }

  function insertVariableAtCursor(text) {
    if (!promptEditorTextarea) return;
    const startPos = promptEditorTextarea.selectionStart;
    const endPos = promptEditorTextarea.selectionEnd;
    const value = promptEditorTextarea.value;
    promptEditorTextarea.value = value.substring(0, startPos) + text + value.substring(endPos, value.length);
    promptEditorTextarea.focus();
    promptEditorTextarea.selectionStart = promptEditorTextarea.selectionEnd = startPos + text.length;
    appConfig.prompts[activePromptTool] = promptEditorTextarea.value;
    updateCharCount();
  }

  function updateCharCount() {
    if (!promptCharCount || !promptEditorTextarea) return;
    const len = promptEditorTextarea.value.length;
    promptCharCount.textContent = `${len} caractère${len > 1 ? 's' : ''}`;
  }
  ...
  if (savePromptsBtn) {
    savePromptsBtn.addEventListener('click', () => {
      ...
      localStorage.setItem('iacademie_custom_prompts', JSON.stringify(appConfig.prompts));
      window.showToast('Directives IA enregistrées avec succès ! ✓');
    });
  }

  if (resetCurrentPromptBtn) {
    resetCurrentPromptBtn.addEventListener('click', () => {
      const def = DEFAULT_SYSTEM_PROMPTS[activePromptTool];
      if (def) {
        appConfig.prompts[activePromptTool] = def;
        if (promptEditorTextarea) promptEditorTextarea.value = def;
        updateCharCount();
        localStorage.setItem('iacademie_custom_prompts', JSON.stringify(appConfig.prompts));
        window.showToast(`Prompt rétabli par défaut pour cet outil ✓`);
      }
    });
  }
  ...
  if (resetAllPromptsBtn) {
    resetAllPromptsBtn.addEventListener('click', () => {
      if (confirm("Voulez-vous réinitialiser tous les prompts d'usine par défaut ?")) {
        appConfig.prompts = { ...DEFAULT_SYSTEM_PROMPTS };
        localStorage.removeItem('iacademie_custom_prompts');
        loadActivePrompt();
        window.showToast("Tous les prompts par défaut ont été rétablis ✓");
      }
    });
  }
  ...
  // Auto-refresh Albert and active provider models in background
  (async () => {
    try {
      const albConfig = PROVIDER_PRESETS.albert;
      const res = await probeModelsEndpoint(albConfig.url, albConfig.apiKey);
      if (res && res.models && res.models.length > 0) {
        modelsCache['albert'] = res.models;
        localStorage.setItem('iacademie_provider_models_cache', JSON.stringify(modelsCache));
        refreshAllDropdownsForProvider('albert');
      }
    } catch (e) {
      console.warn("Background Albert models refresh:", e);
    }
  })();
}
