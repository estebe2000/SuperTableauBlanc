...
export function initTodo() {
  ...
  if (!todoInput || !todoSubmitBtn || !todoListContainer) {
    console.warn("Pas-à-Pas DOM elements not found, skipping.");
    return;
  }

  todoSpiciness.addEventListener('input', (e) => { spicinessVal.textContent = `Niveau ${e.target.value} / 5 🧩`; });
  ...
  function createTodoItemElement(taskText) {
    ...
    item.innerHTML = `
      <div class="todo-item">
        ...
        <div class="todo-item-actions">
          <button class="subdivide-btn" title="Sous-diviser en micro-étapes">🧩 Découper plus</button>
    ...
    subdivideBtn.addEventListener('click', () => {
      ...
      }).catch(err => { alert("Erreur lors du découpage."); }).finally(() => { subdivideBtn.disabled = false; subdivideBtn.innerHTML = `🧩 Découper plus`; });
    });
    return item;
  }
  ...
  todoSubmitBtn.addEventListener('click', () => {
    ...
    const granularityDesc = todoSpiciness.value === '1' ? 'grandes étapes' : todoSpiciness.value === '5' ? 'micro-actions guidées' : 'étapes détaillées';
    let promptTemplate = appConfig.prompts?.todo || `Découpe la tâche suivante en étapes simples. Niveau de détail : {granularity}/5 ({granularity_desc}).\nTâche : "{task}"\nRends uniquement un tableau JSON de chaînes de caractères. Exemple: ["étape 1", "étape 2"].`;
    
    let finalPrompt = promptTemplate
      .replace('{task}', task)
      .replace('{granularity}', todoSpiciness.value)
      .replace('{granularity_desc}', granularityDesc);

    // Fallback if placeholders weren't in custom prompt
    if (!finalPrompt.includes(task)) {
      finalPrompt += `\n\nTâche à découper : "${task}" (Granularité : ${todoSpiciness.value}/5)`;
    }

    makeNonStreamingRequest(finalPrompt, { tool: 'todo' })
    .then(responseText => {
