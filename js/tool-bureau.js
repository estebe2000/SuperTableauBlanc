...
export function initBureau() {
    ...
    const dockRemoteMic = document.getElementById('dockRemoteMic');
    const dockBackground = document.getElementById('dockBackground');
    const dockPlaylist = document.getElementById('dockPlaylist');
    const transcribeMobileMicBtn = document.getElementById('transcribeMobileMicBtn');
    ...
    const showMicQrBtn = document.getElementById('showMicQrBtn');
    const micQrModal = document.getElementById('micQrModal');
    const micQrModalCloseArea = document.getElementById('micQrModalCloseArea');
    const closeMicQrModalBtn = document.getElementById('closeMicQrModalBtn');
    const micQrCodeImg = document.getElementById('micQrCodeImg');
    const micQrLinkUrl = document.getElementById('micQrLinkUrl');
    const micQrLinkCopyContainer = document.getElementById('micQrLinkCopyContainer');

    // Thermometer
    const sessionThermometer = document.getElementById('sessionThermometer');
    ...
    const layoutFreeBtn = document.getElementById('layoutFreeBtn');
    const layoutSplitBtn = document.getElementById('layoutSplitBtn');
    const layoutFocusBtn = document.getElementById('layoutFocusBtn');

    // Flash Quiz
    const dockQuiz = document.getElementById('dockQuiz');
    const teacherQuizPanel = document.getElementById('teacherQuizPanel');
    const closeQuizPanelBtn = document.getElementById('closeQuizPanelBtn');
    const quizCreateView = document.getElementById('quizCreateView');
    const quizLiveView = document.getElementById('quizLiveView');
    ...
    const quizLiveQuestionTitle = document.getElementById('quizLiveQuestionTitle');
    const quizResultsBars = document.getElementById('quizResultsBars');
    const quizTotalAnswers = document.getElementById('quizTotalAnswers');
    const closeLiveQuizBtn = document.getElementById('closeLiveQuizBtn');

    // Course Export
    const exportCourseBtn = document.getElementById('exportCourseBtn');
    const courseExportModal = document.getElementById('courseExportModal');
    const courseExportModalCloseArea = document.getElementById('courseExportModalCloseArea');
    const closeCourseExportModalBtn = document.getElementById('closeCourseExportModalBtn');
    const courseExportPreview = document.getElementById('courseExportPreview');
    const copyCourseExportBtn = document.getElementById('copyCourseExportBtn');
    const downloadCourseExportBtn = document.getElementById('downloadCourseExportBtn');

    // Session Pack (Save & Load Course Templates)
    const openLoadSessionModalBtn = document.getElementById('openLoadSessionModalBtn');
    const saveSessionPackQuickBtn = document.getElementById('saveSessionPackQuickBtn');
    const sessionPackModal = document.getElementById('sessionPackModal');
    const closeSessionPackModalBtn = document.getElementById('closeSessionPackModalBtn');
    const sessionPackModalCloseArea = document.getElementById('sessionPackModalCloseArea');
    const recentPacksList = document.getElementById('recentPacksList');
    const importPackFileInput = document.getElementById('importPackFileInput');
    const savePackTitleInput = document.getElementById('savePackTitleInput');
    const savePackBrowserOnlyBtn = document.getElementById('savePackBrowserOnlyBtn');
    const downloadPackFileBtn = document.getElementById('downloadPackFileBtn');
    const tabBtnRecentPacks = document.getElementById('tabBtnRecentPacks');
    const tabBtnImportPack = document.getElementById('tabBtnImportPack');
    const tabBtnSavePack = document.getElementById('tabBtnSavePack');
    const packTabRecent = document.getElementById('packTabRecent');
    const packTabImport = document.getElementById('packTabImport');
    const packTabSave = document.getElementById('packTabSave');
    ...
    // Student understanding feedback state
    let studentFeedbacks = new Map(); // studentId -> 'green'|'yellow'|'red'

    // Flash Quiz state
    let currentActiveQuiz = null; // { id, question, options: [{id, text, count}], total: 0 }
    ...
    function connectWebSocket() {
        ...
        try {
            ...
            socket.onopen = () => {
                ...
                syncPlaylistData();
                if (sessionThermometer) sessionThermometer.style.display = 'inline-flex';
            ...
            socket.onmessage = async (event) => {
                ...
                if (data.type === 'request-sync') {
                    ...
                    if (currentActiveQuiz) {
                        socket.send(JSON.stringify({
                            type: 'sync-quiz-start',
                            id: currentActiveQuiz.id,
                            question: currentActiveQuiz.question,
                            options: currentActiveQuiz.options
                        }));
                    }
                } else if (data.type === 'request-document-adaptation') {
                    handleStudentAdaptationRequest(data);
                } else if (data.type === 'sync-mic-status') {
                    window.showToast("📱 Micro Smartphone connecté à la session ! ✓");
                } else if (data.type === 'sync-transcript') {
                    // Update live transcript textarea from remote mic
                    if (liveTranscriptTextarea && data.text !== undefined) {
                        liveTranscriptTextarea.value = data.text;
                        liveTranscriptTextarea.scrollTop = liveTranscriptTextarea.scrollHeight;
                        if (floatingTranscribeBar) floatingTranscribeBar.style.display = 'flex';
                    }
                } else if (data.type === 'sync-understanding') {
                    // Handle student understanding feedback
                    if (data.studentId && data.status) {
                        studentFeedbacks.set(data.studentId, data.status);
                        updateThermometerDisplay();
                    }
                } else if (data.type === 'sync-quiz-answer') {
                    // Handle student quiz answer
                    if (currentActiveQuiz && data.quizId === currentActiveQuiz.id) {
                        const opt = currentActiveQuiz.options.find(o => o.id === data.optionId);
                        if (opt) {
                            opt.count = (opt.count || 0) + 1;
                            currentActiveQuiz.totalAnswers = (currentActiveQuiz.totalAnswers || 0) + 1;
                            renderQuizLiveResults();
                        }
                    }
                }
            };
    ...
    function updateThermometerDisplay() {
        if (!sessionThermometer) return;
        sessionThermometer.style.display = 'inline-flex';

        let green = 0, yellow = 0, red = 0;
        studentFeedbacks.forEach(status => {
        ...
        if (total === 0) {
            ...
            if (thermoText) thermoText.textContent = '100%';
            return;
        }

        // Score: green=100%, yellow=50%, red=0%
        const score = Math.round(((green * 100) + (yellow * 50)) / total);
        if (thermoFill) {
            thermoFill.style.width = `${Math.max(8, score)}%`;
            thermoFill.style.background = score >= 70 ? '#10b981' : score >= 40 ? '#f59e0b' : '#ef4444';
        }
        if (thermoText) thermoText.textContent = `${score}%`;
    }
    ...
    function openMicQrModal() {
        if (!sessionCode) {
            startSessionBtn.click();
        }
        const micUrl = `${window.location.origin}/?mode=mic&session=${sessionCode}`;
        if (micQrCodeImg) {
            micQrCodeImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(micUrl)}`;
        }
        if (micQrLinkUrl) {
            micQrLinkUrl.textContent = micUrl;
        }
        micQrModal?.classList.add('show');
    }

    showMicQrBtn?.addEventListener('click', openMicQrModal);
    dockRemoteMic?.addEventListener('click', (e) => {
        e.stopPropagation();
        hideAllFloatingElements();
        openMicQrModal();
    });
    transcribeMobileMicBtn?.addEventListener('click', openMicQrModal);
    ...
    micQrLinkCopyContainer?.addEventListener('click', () => {
        const micUrl = `${window.location.origin}/?mode=mic&session=${sessionCode}`;
        navigator.clipboard.writeText(micUrl)
            .then(() => window.showToast("Lien micro copié ! ✓"))
            .catch(() => alert("Impossible de copier le lien."));
    });
    ...
    const closeMicModal = () => micQrModal?.classList.remove('show');
    closeMicQrModalBtn?.addEventListener('click', closeMicModal);
    micQrModalCloseArea?.addEventListener('click', closeMicModal);

    // ─── QUICK LAYOUT PRESETS ───
    layoutFreeBtn?.addEventListener('click', () => {
        ...
        syncDesktopState();
        window.showToast("Disposition libre réinitialisée ✓");
    });

    layoutSplitBtn?.addEventListener('click', () => {
        if (activeWidgets.length === 0) {
            window.showToast("Aucun widget ouvert pour le split.");
            return;
        }
        if (activeWidgets.length === 1) {
            const w = activeWidgets[0];
            w.x = 5; w.y = 5; w.width = 600; w.height = 450;
            const el = document.getElementById(w.id);
            if (el) {
                el.style.left = '5%'; el.style.top = '5%';
                el.style.width = '90%'; el.style.height = '85%';
            }
        } else {
            // Split 2 widgets side by side
            const w1 = activeWidgets[0];
            const w2 = activeWidgets[1];
            w1.x = 2; w1.y = 5;
            w2.x = 51; w2.y = 5;
            const el1 = document.getElementById(w1.id);
            const el2 = document.getElementById(w2.id);
            if (el1) { el1.style.left = '2%'; el1.style.top = '5%'; el1.style.width = '47%'; el1.style.height = '85%'; }
            if (el2) { el2.style.left = '51%'; el2.style.top = '5%'; el2.style.width = '47%'; el2.style.height = '85%'; }
        }
        syncDesktopState();
        window.showToast("Agencement Split 50/50 appliqué ✓");
    });

    layoutFocusBtn?.addEventListener('click', () => {
        if (activeWidgets.length === 0) {
            window.showToast("Aucun widget à mettre en focus.");
            return;
        }
        const mainW = activeWidgets[activeWidgets.length - 1];
        const el = document.getElementById(mainW.id);
        if (el) {
            el.style.left = '3%';
            el.style.top = '4%';
            el.style.width = '94%';
        ...
        syncDesktopState();
        window.showToast(`Focus sur : ${mainW.title} ✓`);
    });

    // ─── FLASH QUIZ & POLLS LOGIC ───
    dockQuiz?.addEventListener('click', (e) => {
        e.stopPropagation();
        hideAllFloatingElements();
        if (teacherQuizPanel) {
            teacherQuizPanel.style.display = teacherQuizPanel.style.display === 'none' ? 'block' : 'none';
        }
    });

    closeQuizPanelBtn?.addEventListener('click', () => {
        if (teacherQuizPanel) teacherQuizPanel.style.display = 'none';
    });

    const quizHeader = teacherQuizPanel?.querySelector('.panel-header');
    if (teacherQuizPanel && quizHeader) {
        makeDraggable(teacherQuizPanel, quizHeader);
    }

    generateAiQuizBtn?.addEventListener('click', async () => {
        const currentTranscript = liveTranscriptTextarea?.value.trim() || "";
        const prompt = `Tu es Albert, assistant pédagogique. Crée UNE question flash sous forme de QCM (3 options : A, B, C) basée sur le cours suivant :
"""
${currentTranscript || "Les principes de la Conception Universelle de l'Apprentissage (CUA) et de l'accessibilité pédagogique."}
"""

Format STRICT JSON :
{
  "question": "Texte de la question courte et stimulante ?",
  "options": ["Option A", "Option B", "Option C"]
}`;

        generateAiQuizBtn.disabled = true;
        generateAiQuizBtn.textContent = "⏳ Génération...";
        try {
            let resultJson = "";
            ...
            }, (chunk) => { resultJson += chunk; }, (complete) => {
                try {
                    const cleanJson = complete.replace(/```json/g, '').replace(/```/g, '').trim();
                    const parsed = JSON.parse(cleanJson);
                    if (quizQuestionInput) quizQuestionInput.value = parsed.question || "";
                    if (quizOpt1 && parsed.options?.[0]) quizOpt1.value = parsed.options[0];
                    if (quizOpt2 && parsed.options?.[1]) quizOpt2.value = parsed.options[1];
                    if (quizOpt3 && parsed.options?.[2]) quizOpt3.value = parsed.options[2];
                    window.showToast("Question IA générée ✓");
                } catch (e) {
                    console.warn("JSON parse error from Albert quiz:", e);
                    if (quizQuestionInput) quizQuestionInput.value = "Qu'avez-vous retenu du point abordé ?";
                }
                generateAiQuizBtn.disabled = false;
                generateAiQuizBtn.textContent = "🔮 Question IA Flash";
            }, (err) => {
    ...
    launchQuizBtn?.addEventListener('click', () => {
        const qText = quizQuestionInput?.value.trim();
        const o1 = quizOpt1?.value.trim();
        const o2 = quizOpt2?.value.trim();
        const o3 = quizOpt3?.value.trim();

        if (!qText || !o1 || !o2) {
            alert("Veuillez saisir une question et au moins deux options de réponse.");
            return;
        }

        const options = [
            { id: 'opt_a', text: o1, count: 0 },
            { id: 'opt_b', text: o2, count: 0 }
        ];
        if (o3) options.push({ id: 'opt_c', text: o3, count: 0 });
        ...
        if (quizCreateView) quizCreateView.style.display = 'none';
        if (quizLiveView) quizLiveView.style.display = 'block';
        if (quizLiveQuestionTitle) quizLiveQuestionTitle.textContent = qText;

        renderQuizLiveResults();
        window.showToast("Flash Quiz diffusé aux élèves ! 🚀");
    });

    function renderQuizLiveResults() {
        if (!currentActiveQuiz || !quizResultsBars) return;
        quizResultsBars.innerHTML = '';
        if (quizTotalAnswers) quizTotalAnswers.textContent = currentActiveQuiz.totalAnswers || 0;

        const total = currentActiveQuiz.totalAnswers || 0;
        currentActiveQuiz.options.forEach((opt, idx) => {
            ...
            barEl.style = 'background: rgba(255,255,255,0.06); border-radius: 6px; padding: 6px 10px; font-size: 0.78rem; color: white;';
            barEl.innerHTML = `
                <div style="display: flex; justify-content: space-between; margin-bottom: 4px;">
                    <span><strong>${label}.</strong> ${opt.text}</span>
                ...
                <div style="width: 100%; height: 6px; background: rgba(0,0,0,0.3); border-radius: 3px; overflow: hidden;">
                    <div style="width: ${pct}%; height: 100%; background: linear-gradient(90deg, #dc2626, #ea580c); transition: width 0.3s ease;"></div>
                </div>
            `;
            quizResultsBars.appendChild(barEl);
        });
    }

    closeLiveQuizBtn?.addEventListener('click', () => {
        if (socket && socket.readyState === 1 && currentActiveQuiz) {
            socket.send(JSON.stringify({
                type: 'sync-quiz-close',
                id: currentActiveQuiz.id
            }));
        }
        if (quizLiveView) quizLiveView.style.display = 'none';
        if (quizCreateView) quizCreateView.style.display = 'block';
        currentActiveQuiz = null;
        window.showToast("Quiz clôturé ✓");
    });
    ...
    exportCourseBtn?.addEventListener('click', () => {
        ...
        const transcript = liveTranscriptTextarea?.value.trim() || "(Aucune transcription vocale enregistrée)";

        let md = `# Bilan de Cours — All' Inclusive\n\n`;
        md += `**Date :** ${dateStr} à ${timeStr}  \n`;
        md += `**Code de session :** ${sessionCode || 'N/A'}  \n\n`;
        md += `--- \n\n`;

        md += `## 🎙️ Transcription Intégrale du Cours\n\n`;
        md += `${transcript}\n\n`;

        if (playlist && playlist.length > 0) {
            md += `## 📁 Documents & Médias Utilisés\n\n`;
            playlist.forEach(item => {
                md += `- **${item.title}** (${item.type.toUpperCase()}) : ${item.url}\n`;
            });
            md += `\n`;
        }

        if (latestGeneratedDoc) {
            md += `## ✨ Support Adapté CUA Généré\n\n`;
            md += `### ${latestGeneratedDoc.title}\n\n`;
            md += `${latestGeneratedDoc.content}\n\n`;
        }

        if (actionHistory.length > 0) {
            md += `## ⏱️ Déroulé de la Session\n\n`;
            actionHistory.forEach(act => {
                md += `- **${act.time}** : ${act.description}\n`;
            });
        }

        if (courseExportPreview) {
            courseExportPreview.innerHTML = md.replace(/### (.*?)\n/g, '<h3>$1</h3>')
                                             .replace(/## (.*?)\n/g, '<h2>$1</h2>')
                                             .replace(/# (.*?)\n/g, '<h1>$1</h1>')
        ...
        courseExportModal?.classList.add('show');
    });

    const closeCourseModal = () => courseExportModal?.classList.remove('show');
    closeCourseExportModalBtn?.addEventListener('click', closeCourseModal);
    courseExportModalCloseArea?.addEventListener('click', closeCourseModal);

    copyCourseExportBtn?.addEventListener('click', () => {
        const text = courseExportPreview?.innerText || "";
        navigator.clipboard.writeText(text)
            .then(() => window.showToast("Synthèse copiée dans le presse-papier ! ✓"))
            .catch(() => alert("Impossible de copier."));
    });

    downloadCourseExportBtn?.addEventListener('click', () => {
        const text = courseExportPreview?.innerText || "";
        const blob = new Blob([text], { type: 'text/markdown;charset=utf-8' });
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = `Bilan_Cours_All_Inclusive_${sessionCode || 'session'}.md`;
        a.click();
        window.showToast("Fichier de synthèse téléchargé ✓");
    });

    // ─── SESSION PACK (SAVE & LOAD COURSE TEMPLATES) ───
    function openSessionPackModal(defaultTab = 'recent-packs') {
        switchPackTab(defaultTab);
        renderRecentPacksList();
        sessionPackModal?.classList.add('show');
    }

    const closeSessionPackModal = () => sessionPackModal?.classList.remove('show');
    closeSessionPackModalBtn?.addEventListener('click', closeSessionPackModal);
    sessionPackModalCloseArea?.addEventListener('click', closeSessionPackModal);

    openLoadSessionModalBtn?.addEventListener('click', () => openSessionPackModal('recent-packs'));
    saveSessionPackQuickBtn?.addEventListener('click', () => openSessionPackModal('save-pack'));

    function switchPackTab(tabId) {
        document.querySelectorAll('.pack-tab-btn').forEach(btn => {
            btn.classList.toggle('active', btn.getAttribute('data-tab') === tabId);
        });
        if (packTabRecent) packTabRecent.style.display = tabId === 'recent-packs' ? 'block' : 'none';
        if (packTabImport) packTabImport.style.display = tabId === 'import-pack' ? 'block' : 'none';
        if (packTabSave) packTabSave.style.display = tabId === 'save-pack' ? 'block' : 'none';
    }

    tabBtnRecentPacks?.addEventListener('click', () => switchPackTab('recent-packs'));
    tabBtnImportPack?.addEventListener('click', () => switchPackTab('import-pack'));
    tabBtnSavePack?.addEventListener('click', () => switchPackTab('save-pack'));

    function buildCurrentSessionPack(title) {
        return {
            format: 'all-inclusive-pack',
            version: '1.0',
            id: `pack-${Date.now()}`,
            title: title || `Séance du ${new Date().toLocaleDateString('fr-FR')}`,
            createdAt: new Date().toISOString(),
            dateFormatted: new Date().toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
            background: backgroundStyle,
            widgets: activeWidgets.map(w => ({ ...w })),
            playlist: (playlist || []).map(p => ({ ...p })),
            drawingPaths: [...drawingPaths],
            quizData: currentActiveQuiz ? { ...currentActiveQuiz } : null
        };
    }

    function savePackToLocalStorage(pack) {
        try {
            const raw = localStorage.getItem('stored_session_packs');
            let packs = raw ? JSON.parse(raw) : [];
            // Remove older pack with same id if exists, add new to beginning
            packs = packs.filter(p => p.id !== pack.id);
            packs.unshift(pack);
            // Limit to 15 stored packs
            if (packs.length > 15) packs = packs.slice(0, 15);
            localStorage.setItem('stored_session_packs', JSON.stringify(packs));
            renderRecentPacksList();
        } catch (e) {
            console.warn("Failed saving pack to localStorage:", e);
        }
    }

    function renderRecentPacksList() {
        if (!recentPacksList) return;
        try {
            const raw = localStorage.getItem('stored_session_packs');
            const packs = raw ? JSON.parse(raw) : [];

            if (packs.length === 0) {
                recentPacksList.innerHTML = `<p style="color: var(--text-muted); font-size: 0.85rem; text-align: center; padding: 20px;">Aucune séance mémorisée dans ce navigateur pour le moment.</p>`;
                return;
            }

            recentPacksList.innerHTML = '';
            packs.forEach(pack => {
                const item = document.createElement('div');
                item.className = 'recent-pack-item';
                item.style = 'display: flex; justify-content: space-between; align-items: center; background: var(--surface2); border: 1px solid var(--border); border-radius: 8px; padding: 10px 14px;';
                
                const widgetCount = pack.widgets?.length || 0;
                const mediaCount = pack.playlist?.length || 0;
                
                item.innerHTML = `
                    <div style="flex: 1; min-width: 0; margin-right: 12px;">
                        <h4 style="margin: 0 0 4px 0; font-size: 0.95rem; color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${pack.title}</h4>
                        <div style="font-size: 0.75rem; color: var(--text-muted); display: flex; gap: 10px;">
                            <span>🕒 ${pack.dateFormatted || 'Date inconnue'}</span>
                            <span>📦 ${widgetCount} widget(s)</span>
                            <span>🎥 ${mediaCount} média(s)</span>
                        </div>
                    </div>
                    <div style="display: flex; gap: 6px;">
                        <button class="btn btn-primary btn-sm btn-load-pack" style="padding: 4px 10px; font-size: 0.78rem;">📂 Charger</button>
                        <button class="btn btn-secondary btn-sm btn-dl-pack" title="Télécharger le fichier" style="padding: 4px 8px; font-size: 0.78rem;">📥</button>
                        <button class="btn btn-danger btn-sm btn-del-pack" title="Supprimer de l'historique" style="padding: 4px 8px; font-size: 0.78rem;">🗑️</button>
                    </div>
                `;

                item.querySelector('.btn-load-pack').addEventListener('click', () => {
                    applySessionPack(pack);
                    closeSessionPackModal();
                });

                item.querySelector('.btn-dl-pack').addEventListener('click', () => {
                    downloadPackJson(pack);
                });

                item.querySelector('.btn-del-pack').addEventListener('click', () => {
                    if (confirm(`Supprimer "${pack.title}" de vos séances mémorisées ?`)) {
                        deletePackFromLocalStorage(pack.id);
                    }
                });

                recentPacksList.appendChild(item);
            });
        } catch (e) {
            console.error("Error rendering recent packs:", e);
        }
    }

    function deletePackFromLocalStorage(packId) {
        try {
            const raw = localStorage.getItem('stored_session_packs');
            let packs = raw ? JSON.parse(raw) : [];
            ...
            localStorage.setItem('stored_session_packs', JSON.stringify(packs));
            renderRecentPacksList();
            window.showToast?.("Séance supprimée de l'historique ✓");
        } catch (e) {}
    }
    ...
    function downloadPackJson(pack) {
        ...
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        const safeTitle = (pack.title || 'Seance').replace(/[^a-zA-Z0-9_\-]/g, '_');
        a.download = `${safeTitle}.stb`;
        a.click();
        window.showToast?.("Pack de séance (.stb) téléchargé ! ✓");
    }

    function applySessionPack(packData) {
        if (!packData) return;

        try {
            // 1. Clear current widgets from desktop
            bureauDesktop.querySelectorAll('.widget-instance').forEach(w => w.remove());
            activeWidgets = [];

            // 2. Restore background
            if (packData.background) {
                backgroundStyle = packData.background;
                applyBackgroundLocal(backgroundStyle);
                syncBackground(backgroundStyle);
            }

            // 3. Restore playlist
            if (packData.playlist && Array.isArray(packData.playlist)) {
                playlist = [...packData.playlist];
                updatePlaylistDOM();
            }

            // 4. Restore widgets
            if (packData.widgets && Array.isArray(packData.widgets)) {
                packData.widgets.forEach(w => {
                    const copy = { ...w };
                    activeWidgets.push(copy);
                    renderWidgetOnTeacherDesktop(copy);
                });
                syncDesktopState();
            }

            // 5. Restore drawings if present
            if (packData.drawingPaths && Array.isArray(packData.drawingPaths)) {
                drawingPaths = [...packData.drawingPaths];
                redrawCanvas();
                syncDesktopState();
            }
            ...
            // Also save to recent packs if not already there
            savePackToLocalStorage(packData);

            logAction(`Chargement de la séance : ${packData.title}`);
            window.showToast(`Séance "${packData.title}" restaurée sur le tableau ! 🚀`);
        } catch (err) {
            console.error("Failed to apply session pack:", err);
            alert(`Erreur lors de l'application de la séance : ${err.message}`);
        }
    }

    savePackBrowserOnlyBtn?.addEventListener('click', () => {
        const title = savePackTitleInput?.value.trim() || `Séance du ${new Date().toLocaleDateString('fr-FR')}`;
        const pack = buildCurrentSessionPack(title);
        savePackToLocalStorage(pack);
        window.showToast("Séance mémorisée dans le navigateur ! ✓");
        switchPackTab('recent-packs');
    });

    downloadPackFileBtn?.addEventListener('click', () => {
        const title = savePackTitleInput?.value.trim() || `Séance du ${new Date().toLocaleDateString('fr-FR')}`;
        const pack = buildCurrentSessionPack(title);
        savePackToLocalStorage(pack);
        downloadPackJson(pack);
    ...
    importPackFileInput?.addEventListener('change', (e) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (!parsed.widgets && !parsed.playlist) {
                    alert("Fichier de séance invalide ou corrompu.");
                    return;
                }
                applySessionPack(parsed);
                closeSessionPackModal();
            } catch (err) {
                console.error("Failed to parse pack file:", err);
                alert("Erreur lors de la lecture du fichier de séance.");
            }
        };
...
- Sépare bien les paragraphes et évite les explications trop denses.`;
            ...
            base += `\nProduis une CARTE MENTALE RADIALE ÉPURÉE au format Mermaid mindmap.
CONSIGNES STRICTES :
1. Démarre STRICTEMENT par le mot-clé "mindmap".
2. Définis un nœud central root((Sujet Principal)) (2 mots max).
3. 3 à 5 branches principales, feuilles très concises (1 à 4 mots max) entourées de guillemets [\"...\"]`;
