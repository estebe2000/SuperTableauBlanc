// Fichier: modules/ia/prompts/markthewords-prompt.js

import { logger } from '../../utils/logger.js';
import { getBOPromptBlockForCurrentSelection } from '../bo-context.js';

export async function preparerAssistantIA_MarkWords(rules, sourceText) {
    ...
    // ARCHITECTURE « liste + texte nu » : on NE demande PLUS à l'IA d'insérer des
    // marqueurs dans le texte (trop fragile : l'IA en oublie, et n8n strippe les
    // astérisques). L'IA renvoie séparément la LISTE des mots cibles et le texte
    // recopié SANS marqueur ; le balisage *mot* (format H5P) est fait côté client
    // par handleParseIA, ce qui garantit l'exhaustivité et survit à n8n.
    let prompt = `Tu es un assistant pédagogique expert en linguistique.\n`;
    prompt += `Analyse le texte fourni ci-dessous (entre guillemets triples) et identifie TOUS les mots ISOLÉS (un seul mot, JAMAIS un groupe de mots) qui correspondent à L'UN des critères suivants :\n\n`;
    ...
    prompt += `- ⚠️ RÈGLE TECHNIQUE STRICTE : le composant BUGUE sur les groupes de mots et les articles élidés. Chaque mot cible doit être UN SEUL mot, sans AUCUNE espace. JAMAIS de groupe de mots ni d'expression à plusieurs mots ("lac de retenue", "réseaux sociaux", "réalité virtuelle"), et JAMAIS l'article avec apostrophe : mets "eau" et non "l'eau". Si un concept clé est une expression de plusieurs mots, NE la sélectionne PAS en entier : choisis UN SEUL mot significatif de l'expression (ex: "virtuelle" seul, ou "réseaux" seul). Avant de répondre, VÉRIFIE que chaque mot de la ligne MOTS: ne contient aucune espace.\n`;
    ...
    const promptTextarea = document.getElementById('markthewords-ia-prompt');
    if (promptTextarea) {
        promptTextarea.value = prompt;
        logger.log('🤖 Prompt (Mark the Words) finalisé avec combinaisons.');
    }
    
    // ✅ Il faut absolument retourner "true" pour déclencher l'affichage dans l'interface !
    return true;
}